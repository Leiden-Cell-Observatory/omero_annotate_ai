...
def _(mo):
    ...
    This notebook guides you through annotating images in OMERO using the CellPose workflow.

    **Workflow Steps:**
    1. Connect to your OMERO server
    2. Select a container (project, dataset, or plate)
    3. Configure annotation parameters
    4. Export images for CellPose annotation
    5. Annotate using CellPose GUI
    6. Collect annotations and upload to OMERO
...
def _(mo):
    ...
    return batch_size, read_only_mode
...
def _(Path, datetime, mo):
    output_directory = mo.ui.text(
        value=str(Path.home() / "omero_annotate_ai" / "omero_annotations_cellpose"),
        label="Output Directory",
        full_width=True,
    )
    workflow_name = mo.ui.text(
        value=f"cellpose_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        label="Workflow Name",
    )
...
def _(mo):
    mo.md("""
    ## 7. Export Images for CellPose

    This will download images from OMERO and save them locally for CellPose annotation.
    """)
    return
...
@app.cell
def _(mo):
    export_button = mo.ui.run_button(label="Export Images for CellPose", kind="success")
    export_button
    return (export_button,)


@app.cell
def _(config, conn, container_id, create_pipeline, export_button, mo):
    mo.stop(not export_button.value, mo.md("Click 'Export Images for CellPose' to start"))
    ...
    if not conn or not conn.isConnected():
        ...
        try:
            ...
            mo.output.replace(mo.md("**Exporting images for CellPose annotation...**"))
            ...
            **Next steps:**
            1. Open CellPose GUI: `pixi run cellpose` or `python -m cellpose`
            2. Open the exported images and annotate them
            3. Save masks to the output folders
            4. Return here and click 'Collect Annotations'
            """))
        except Exception as e:
            mo.output.replace(mo.md(f"**Error:** {str(e)}"))
    return pipeline, table_id


@app.cell
def _(mo):
    mo.md("""
    ## 8. Collect Annotations

    After annotating in CellPose, collect the masks and upload them to OMERO.
    """)
    return
...
@app.cell
def _(mo):
    collect_button = mo.ui.run_button(label="Collect Annotations from Disk")
    collect_button
    return (collect_button,)


@app.cell
def _(collect_button, mo, pipeline):
    mo.stop(not collect_button.value, mo.md("Click 'Collect Annotations' after CellPose annotation"))

    if pipeline is None:
        mo.output.replace(mo.md("**Error:** Run the export step first"))
    else:
        try:
            ...
            if status['available_masks']:
                ...
                **Annotations collected!**
                ...
                - Processed: {uploaded_count} annotations
                - Progress: {progress['completed_units']}/{progress['total_units']} ({progress['progress_percent']}%)
                """))
            else:
                mo.output.replace(mo.md("No mask files found yet. Annotate images in CellPose first."))
        except Exception as e:
            mo.output.replace(mo.md(f"**Error:** {str(e)}"))
    return
