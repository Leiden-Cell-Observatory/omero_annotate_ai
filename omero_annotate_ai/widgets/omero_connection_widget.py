...
class OMEROConnectionWidget:
    ...
    def _create_widgets(self):
        ...
        self.connect_button = widgets.Button(
            description='Save & Connect',
            button_style='success',
            icon='check'
        )
        
        self.load_keychain_button = widgets.Button(
            description='Load from Keychain',
            button_style='',
            icon='key'
        )
    ...
    def _setup_observers(self):
        ...
        # Show/hide password
        self.show_password_widget.observe(self._toggle_password_visibility, names='value')
    ...
    def _load_existing_config(self):
        ...
        if config:
            ...
            self.config_info.value = f"<i>🔄 Pre-populated from {' + '.join(sources)}</i>"
        else:
    ...
    def _populate_connection_dropdown(self):
        ...
        for conn in connections:
            display_text = f"{conn['display_name']} (last used: {conn['last_used_display']})"
        ...
        if current_host and current_username:
            ...
            for display_text, conn in options[1:]:  # Skip 'Manual entry'
                if conn and conn['host'] == current_host and conn['username'] == current_username:
    ...
    def _save_and_connect(self, button):
        ...
        with self.status_output:
            ...
            self.connection = self.connection_manager.create_connection_from_config(config)
    ...
    def _save_connection_only(self, button):
        ...
        with self.status_output:
            ...
            success = self.connection_manager.save_connection_details(host, username, group)
            ...
            if success:
                ...
                if self.save_password_widget.value:
                    ...
                    if password:
                        expire_hours = self.expire_widget.value
                        self.connection_manager.save_password(host, username, password, expire_hours)
