...
def generate_patch_coordinates(image_shape: Tuple[int, int], patch_size: Tuple[int, int], 
                              n_patches: int, random_patch: bool = True) -> List[Tuple[int, int]]:
    ...
    if random_patch:
        ...
        if len(coordinates) < n_patches:
            print(f"⚠️ Could only place {len(coordinates)} non-overlapping patches out of {n_patches} requested")
        ...
        if n_patches > max_grid_patches:
            print(f"⚠️ Requested {n_patches} patches, but only {max_grid_patches} non-overlapping patches fit")
