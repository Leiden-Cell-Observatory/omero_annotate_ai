...
from ..omero.omero_functions import list_annotation_tables, generate_unique_table_name
...
class WorkflowWidget:
    ...
    def __init__(self, connection=None):
        ...
        self.current_step = 0 if connection is None else 1  # Skip connection step if provided
        self.steps = [
    ...
    def _create_config_tab(self):
        ...
        config_info = widgets.HTML(
            value="<p>Configure your annotation parameters:</p>"
        )
    ...
    def _create_annotation_settings(self):
        ...
        segment_all = widgets.Checkbox(
            ...
            description='Annotate all images'
        )
        ...
        timepoint_mode = widgets.Dropdown(
            options=[('Specific timepoints', 'specific'), ('All timepoints', 'all'), ('Random selection', 'random')],
            value=self.config.microsam.timepoint_mode,
            description='Timepoints:'
        )
        
        timepoints = widgets.Text(
            value=', '.join(map(str, self.config.microsam.timepoints)),
        ...
        z_slice_mode = widgets.Dropdown(
            options=[('Specific slices', 'specific'), ('All slices', 'all'), ('Random selection', 'random')],
            value=self.config.microsam.z_slice_mode,
            description='Z-slices:'
        )
        
        z_slices = widgets.Text(
            value=', '.join(map(str, self.config.microsam.z_slices)),
        ...
        # Setup observers
        segment_all.observe(lambda c: self._toggle_subset_settings(c, train_n, validate_n), names='value')
        timepoint_mode.observe(lambda c: self._toggle_list_setting(c, timepoints), names='value')
        z_slice_mode.observe(lambda c: self._toggle_list_setting(c, z_slices), names='value')
        use_patches.observe(lambda c: self._toggle_patch_settings(c, patches_per_image, patch_size), names='value')
    ...
    def _create_technical_settings(self):
        ...
        model_type = widgets.Dropdown(
            options=[('vit_b_lm (recommended)', 'vit_b_lm'), ('vit_b', 'vit_b'), ('vit_l', 'vit_l'), ('vit_h', 'vit_h')],
    ...
    def _create_save_tab(self):
        ...
        save_info = widgets.HTML(
            value="<p>Review and save your configuration:</p>"
        )
    ...
    def _can_proceed_to_next(self):
        ...
        if self.connection is None:
            ...
            if self.current_step == 0:  # Connection step
                ...
                return self.selected_table_id is not None or self.new_table_name.value.strip()
    ...
    def _load_containers(self):
        ...
        try:
            ...
            for container in containers:
                options.append((f"{container.getName()} (ID: {container.getId()})", container.getId()))
    ...
    def _on_scan_tables(self, button):
        ...
        try:
            ...
            self.annotation_tables = list_annotation_tables(self.connection, container_type, container_id)
            ...
            with self.tables_widgets['display']:
                ...
                if self.annotation_tables:
                    ...
                    for i, table in enumerate(self.annotation_tables):
                        print(f"{i+1}. {table.get('name', 'Unknown')} (ID: {table.get('id')})")
    ...
    def _on_new_table(self, button):
        ...
        try:
            ...
            unique_name = generate_unique_table_name(self.connection, container_type, container_id, base_name)
    ...
    def _on_update_config(self, button):
        ...
        try:
            ...
            if container_id:
                ...
                try:
                    container_obj = self.connection.getObject(container_type.capitalize(), container_id)
                    if container_obj:
                ...
                if self.selected_table_id:
                    ...
                    # Creating new table
                    self.config.training.trainingset_name = self.new_table_name.value.strip()
                    self.config.workflow.resume_from_table = False
            ...
            with self.status_output:
                ...
                print(f"📊 Container: {self.config.omero.container_type} (ID: {self.config.omero.container_id})")
                print(f"🎯 Training Set: {self.config.training.trainingset_name}")
                print(f"🔬 Model: {self.config.microsam.model_type}")
    ...
    def _update_annotation_settings(self):
        ...
        if 'timepoint_mode' in annotation_widgets:
            self.config.microsam.timepoint_mode = annotation_widgets['timepoint_mode'].value
        if 'timepoints' in annotation_widgets:
            timepoints_str = annotation_widgets['timepoints'].value
            try:
                self.config.microsam.timepoints = [int(x.strip()) for x in timepoints_str.split(',') if x.strip()]
            except:
                self.config.microsam.timepoints = [0]
        if 'z_slice_mode' in annotation_widgets:
            self.config.microsam.z_slice_mode = annotation_widgets['z_slice_mode'].value
        if 'z_slices' in annotation_widgets:
            z_slices_str = annotation_widgets['z_slices'].value
            try:
                self.config.microsam.z_slices = [int(x.strip()) for x in z_slices_str.split(',') if x.strip()]
            except:
                self.config.microsam.z_slices = [0]
        if 'three_d' in annotation_widgets:
            self.config.microsam.three_d = annotation_widgets['three_d'].value
        ...
        if 'patches_per_image' in annotation_widgets:
            self.config.patches.patches_per_image = annotation_widgets['patches_per_image'].value
    ...
    def _update_technical_settings(self):
        ...
        if 'batch_size' in technical_widgets:
            self.config.batch_processing.batch_size = technical_widgets['batch_size'].value
        if 'model_type' in technical_widgets:
            self.config.microsam.model_type = technical_widgets['model_type'].value
        if 'output_folder' in technical_widgets:
            self.config.batch_processing.output_folder = technical_widgets['output_folder'].value
    ...
    def _update_workflow_settings(self):
        ...
        if 'read_only_mode' in workflow_widgets:
            self.config.workflow.read_only_mode = workflow_widgets['read_only_mode'].value
        if 'resume_from_table' in workflow_widgets:
            self.config.workflow.resume_from_table = workflow_widgets['resume_from_table'].value
