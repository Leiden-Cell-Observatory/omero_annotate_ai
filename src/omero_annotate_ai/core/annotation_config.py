...
def _annotation_id_base(
    image_id: int, timepoint: int, z_slice: int, is_volumetric: bool
) -> str:
    """Base annotation id (without patch index) as generated by the pipeline."""
    if is_volumetric:
        return f"{image_id}_{timepoint}_3d"
    return f"{image_id}_{timepoint}_{z_slice}"


def _regenerate_annotation_ids(df: pd.DataFrame) -> List[str]:
    """Rebuild the pipeline's annotation ids for a table, positionally.

    Legacy OMERO tables were written before ``annotation_id`` was persisted, so the
    id has to be reconstructed from the row's coordinates. This follows the scheme
    used by ``AnnotationPipeline`` when the annotations were first created:

    - volumetric patch:    ``{image_id}_{timepoint}_3d_{patch_index}``
    - volumetric, no patch:``{image_id}_{timepoint}_3d``
    - 2D patch:            ``{image_id}_{timepoint}_{z_slice}_{patch_index}``
    - 2D, no patch:        ``{image_id}_{timepoint}_{z_slice}``

    ``patch_index`` is not stored in the table, so it is derived deterministically:
    patch rows sharing a base id are sorted on ``(patch_x, patch_y)`` (ties broken by
    row order) and take their 0-based position in that ordering.

    Args:
        df: OMERO table DataFrame.

    Returns:
        List of ids, one per row, in DataFrame row order.
    """
    bases: List[str] = []
    patch_flags: List[bool] = []
    patch_groups: Dict[str, List[Tuple[int, int, int]]] = {}

    for position, (_, row) in enumerate(df.iterrows()):
        base = _annotation_id_base(
            _row_int(row, "image_id"),
            _row_int(row, "timepoint"),
            _row_int(row, "z_slice"),
            bool(row.get("is_volumetric", False)),
        )
        is_patch = bool(row.get("is_patch", False))
        bases.append(base)
        patch_flags.append(is_patch)
        if is_patch:
            patch_groups.setdefault(base, []).append(
                (
                    _row_int(row, "patch_x", 0),
                    _row_int(row, "patch_y", 0),
                    position,
                )
            )

    # Within each base group, sort patches on (patch_x, patch_y) -> 0-based index
    patch_indices: Dict[int, int] = {}
    for entries in patch_groups.values():
        for index, (_x, _y, position) in enumerate(sorted(entries)):
            patch_indices[position] = index

    return [
        f"{base}_{patch_indices[position]}" if patch_flags[position] else base
        for position, base in enumerate(bases)
    ]


def _uniquify_annotation_ids(ids: List[str]) -> List[str]:
    """Make ids unique by suffixing repeats, preserving order and determinism."""
    used: set = set()
    unique: List[str] = []
    for value in ids:
        candidate = value
...
def _resolve_annotation_ids(df: pd.DataFrame) -> List[str]:
    """Resolve one non-empty, unique annotation id per row of an OMERO table.

    Uses the stored ``annotation_id`` column where present and non-empty, and
    regenerates the id from the row coordinates otherwise (legacy tables).
    """
    fallback = _regenerate_annotation_ids(df)

    resolved: List[str] = []
    has_column = "annotation_id" in df.columns
    for position, (_, row) in enumerate(df.iterrows()):
        stored = ""
        if has_column:
            value = row.get("annotation_id", "")
            try:
...
class AnnotationConfig(BaseModel):
    ...
    def from_dataframe(self, df: pd.DataFrame):
        ...
        Every reconstructed annotation gets a non-empty, unique ``annotation_id``:
        taken from the ``annotation_id`` column when the table has one, and
        regenerated deterministically from the row coordinates otherwise (legacy
        tables written before the column existed). See
        :func:`_regenerate_annotation_ids`.
        ...
        annotation_ids = _resolve_annotation_ids(df)
        ...
        for position, (_, row) in enumerate(df.iterrows()):
            ...
            annotation_data = {
                ...
                "annotation_id": annotation_ids[position],
